...
async fn read_all(data: web::Data<Arc<Mutex<AppState>>>) -> impl Responder {
    ...
    let result = actix_web::rt::task::spawn_blocking(move || {
        ...
        // Tension max enregistrée : UNIQUEMENT Phase A pour chaque source (demande utilisateur)
        if let Some(v) = read_register(&guard, addr, 0x000F, debug) {
            values.insert("max1".to_string(), format!("{} V", v));
        }
        if let Some(v) = read_register(&guard, addr, 0x0012, debug) {
            values.insert("max2".to_string(), format!("{} V", v));
        }
        ...
        if let Some(power) = read_register(&guard, addr, 0x004F, debug) {
            ...
            // === BITS CORRIGÉS SELON MANUEL CHINT 2025 ===
            // Bit 3 = SW1 (Onduleur)
            // Bit 4 = SW2 (Réseau)
            // Bit 1 = Mode Auto / Manuel
            // Bit 8 = Télécommande
            // Position centrale (middleOFF) = les DEUX bits SW1 et SW2 sont à 0

            let sw1_bit = (switch & 0x0008) != 0;
            let sw2_bit = (switch & 0x0010) != 0;
            let middle_off = !sw1_bit && !sw2_bit;   // Les deux bits à 0 = position centrale

            // Correction pour la position milieu : forcer les deux en "Ouvert"
            values.insert("sw1".to_string(),
                if middle_off {
                    "⭕ Ouvert"
                } else if sw1_bit {
                    "⭕ Ouvert"
            ...
            // Affichage de la position centrale
            values.insert("middleOFF".to_string(),
                if middle_off {
                    "✅ Activé (position centrale / double déclenché)"
                } else {
                    "⭕ Désactivé"
                }.to_string());

            values.insert("swMode".to_string(),
                if (switch & 0x0001) != 0 { "🤖 Auto" } else { "👆 Manuel" }.to_string());

            values.insert("swRemote".to_string(),
                if (switch & 0x0100) != 0 { "📡 Activé" } else { "🔒 Désactivé" }.to_string());

            let fault = (switch >> 5) & 0x07;
            values.insert("swFault".to_string(), match fault {
                0 => "Aucun",
                1 => "Interconnexion incendie",
                2 => "Surcharge du moteur",
                3 => "Disjonction I Onduleur",
                4 => "Disjonction II Réseau",
                5 => "Signal de fermeture anormal",
                6 => "Phase anormal I",
                7 => "Phase anormal II",
                _ => "Inconnu"
            }.to_string());
