...
Remplacement total de la stack Python/FastAPI par **Rust** (workspace multi-crates : `daly-bms-core` + `daly-bms-server` + `daly-bms-cli` + `daly-bms-probe` + `daly-bms-venus`).
...
│                                                                    │
│   daly-bms-venus (Rust) ◄── MQTT santuario/bms/{n}/venus         │
│   [TODO] venus-solar    ◄── MQTT santuario/solar/{n}/venus        │
│   [TODO] venus-meteo    ◄── MQTT santuario/meteo/venus            │
│   [TODO] venus-ats      ◄── MQTT santuario/ats/venus              │
│                │                                                   │
│                ▼  D-Bus (zbus / pur Rust)                          │
...
| Topic MQTT | Source | Cible D-Bus Venus |
|---|---|---|
| `santuario/bms/{n}/venus` | daly-bms-server | `com.victronenergy.battery.{n}` |
| `santuario/solar/{n}/venus` | [TODO] daly-bms-solar | `com.victronenergy.meteo.{n}` |
| `santuario/meteo/venus` | [TODO] daly-bms-solar | `com.victronenergy.meteo` |
| `santuario/heat/{n}/venus` | [TODO] daly-bms-heatpump | `com.victronenergy.temperature.{n}` |
| `santuario/ats/venus` | [TODO] daly-bms-ats | `com.victronenergy.grid` |
...
| **daly-bms-venus** | NanoPi | — | Bridge MQTT → D-Bus Venus OS (Rust pur, zbus) — enregistre tous les capteurs sur Venus |

> **Note architecture** : Le Pi5 est le **master** de tous les capteurs RS485 et API cloud.
> Le NanoPi reste dédié à Venus OS et héberge uniquement `daly-bms-venus` (Rust statique, ~3 Mo).
...
RS485 Bus 2 ──► [TODO] daly-bms-solar::poll_loop()
                         ...
                         MqttBridge ──► santuario/solar/{n}/venus
                                               │
                              daly-bms-venus solar extension
...
| Crate / Binaire        | Statut | Rôle |
|------------------------|--------|------|
| `daly-bms-core`        | ✅ Production | Protocole UART, parsing trames, types (`BmsSnapshot`), bus partagé, commandes lecture/écriture, polling |
| `daly-bms-server`      | ✅ Production | API Axum (REST + WebSocket) + Dashboard SSR + ring buffer + simulateur + bridges (MQTT, InfluxDB, Alertes) |
| `daly-bms-venus`       | ✅ Production | Bridge MQTT → D-Bus Venus OS (zbus pur Rust) — enregistre les batteries sur `com.victronenergy.battery.*` |
| `daly-bms-cli`         | ✅ Stable | Outil CLI de diagnostic et contrôle RS485 |
| `daly-bms-probe`       | ✅ Stable | Outil diagnostic bas niveau — envoie des trames brutes, teste 3 variantes d'adressage |
| `daly-bms-solar`       | 🔜 TODO | Polling RS485 capteur irradiance + station météo → MQTT → D-Bus `com.victronenergy.meteo` |
| `daly-bms-heatpump`    | 🔜 TODO | Bridge API LG Cloud → MQTT → D-Bus `com.victronenergy.temperature` (PAC chauffe-eau + clim) |
| `daly-bms-ats`         | 🔜 TODO | Pilotage ATS RS485 (bascule maison / grid / Victron) → MQTT → D-Bus `com.victronenergy.grid` |
...
│   │       ├── mqtt_source.rs ← Subscriber rumqttc
...
|----------------------------|-------------|-------|
| daly-bms-venus (Rust)      | ~5 MB       | Binaire statique musl, zéro dépendance système |
...
- [x] Crate `daly-bms-venus` : bridge MQTT → D-Bus (zbus pur Rust, sans libdbus)
...
- [ ] Nettoyage NanoPi : services Python retirés, seul `daly-bms-venus` reste
...
- [ ] Créer crate `daly-bms-solar` (polling RS485, types `SolarSnapshot`, `MeteoSnapshot`)
- [ ] Bridge MQTT : topics `santuario/solar/{n}/venus` et `santuario/meteo/venus`
- [ ] Extension `daly-bms-venus` pour `com.victronenergy.meteo.*`
...
- [ ] Créer crate `daly-bms-heatpump` (poller API LG Cloud toutes les 60s)
...
- [ ] Extension `daly-bms-venus` pour `com.victronenergy.temperature.dhw`
...
- [ ] Extension `com.victronenergy.temperature.ac_{zone}`
...
- [ ] Créer crate `daly-bms-ats` (polling état + commandes bascule)
- [ ] Types : `AtsSnapshot` (source active, tensions, fréquence, défauts)
- [ ] Bridge MQTT : `santuario/ats/venus` + commandes `santuario/ats/cmd`
- [ ] Extension `daly-bms-venus` pour `com.victronenergy.grid` (source active)
...
- [ ] Interface unifiée `SensorTrait` pour tous les capteurs (BMS, solaire, PAC, ATS)
